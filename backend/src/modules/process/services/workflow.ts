...
class WorkFlowSchemaService {
    ...
    async updateWorkFlow(process: any, prevWorkflowName:string) {
        try {
            ...
            logger.error('Failed to insert workflow to workflow collection',error);
            return Promise.reject(error)
        }
    }
    ...
    async extractNodesNEdgesFromProcess(process: any) {
        try {
            ...
            if(lastNode && lastNode !== null && !process.isPredefined) { 
            ...
            for await (const key of keys) {
                ...
                console.log("State",state,"stateTransition",stateTransition,"fromState",fromState)
            ...
            logger.error(`Failed to extract node and edges from process data for process:${process.id}`,error);
