...
import { nfc, CHUNKSIZEUPLOAD, tagAlreadyExist, tagDeleted, tagNotFound, BatchNotFound, tagUploadSuccessfully, TRACKNTRACE, DIGITIZATION, FullCount, TransferASN, RecievedASN, RecievedBlind, BulkInsertLimit, processAlreadyRunningInZone, processStateNotFount, encodeMaxCount, NFC } from "../utils/constant";
...
export class TagService {
    ...
    async bulkUploadTags(req: any, xlsxData: any) {
        ...
        try {
            ...
            if (!xlsxData.length) {
                throw "Data not available in uploaded file.";
            }
            else {
                let batchId_;
                let tagsData = [], total = 0;
                tagsData = xlsxData.filter((obj: any) => obj['manufacturerName'] !== undefined || obj['customerName'] !== undefined || obj['batchId'] !== undefined || obj['tagId'] !== undefined || obj['tagInfo'] !== undefined || obj['tagType'] !== undefined || obj['tagClass'] !== undefined || obj['hash'] !== undefined || obj['secureKey'] !== undefined || obj['inlayItemName'] !== undefined || obj['inlayType'] !== undefined || obj['vendorName'] !== undefined || obj['orderId'] !== undefined || obj['orderDate'] !== undefined || obj['orderQuantity'] !== undefined || obj['orderQuantityUnit'] !== undefined || obj['deliveryDate'] !== undefined || obj['deliveryItemName'] !== undefined || obj['deliveryQuantity'] !== undefined || obj['deliveryQuantityUnit'] !== undefined).map(({ manufacturerName = "", customerName = "", batchId = "", tagId = "", tagInfo = "", tagType = "", tagClass = "", hash = "", secureKey = "", inlayItemName = "", inlayType = "", vendorName = "", orderId = "", orderDate = "", orderQuantity = "", orderQuantityUnit = "", deliveryDate = "", deliveryItemName = "", deliveryQuantity = "", deliveryQuantityUnit = "", operationTime = "" }) => ({
                    tenantId,
                    tenantName,
                    userId,
                    userName,
                ...
                for (var i = 0; i < tagsData.length; i++) {
                    let obj = tagsData[i];
                    if ( obj.manufacturerName === '' ||  obj.tagId === "" ||   obj.batchId === "" || obj.tagType === "" ) {
                        logger.debug("obj=>",obj.tagId)
                        let validationDataMessage = {
                            tagId: obj.tagId,
                ...
                logger.debug("Sending data for validation createUniqueTags with validTags data length => ",validTags.length, new Date())
                let dataForInsertTY = await this.createUniqueTags(validTags);
                let { dataForInsert, secureKeyCount, standardKeyCount, addedData, errorMsgData, batchId }: any = dataForInsertTY;
                logger.debug("dataForInsert=>", dataForInsert.length, "secureKeyCount=>", secureKeyCount, "standardKeyCount=>", standardKeyCount, "addedData=>", addedData.length, " errorMsgData=>", errorMsgData.length, "batchId=>", batchId)
                batchId_ = batchId;
                duplicateData = errorMsgData;
                const TOTALDATACOUNT = dataForInsert?.length;
                const CHUNKSIZE = BulkInsertLimit;
                let OFFSET = 0;
                logger.debug("Start Insertion============================>", new Date());
                // tag insertion
                for (let start = 0; start < TOTALDATACOUNT; start++) {

                    if (CHUNKSIZE * OFFSET === start) {
                        let end = start + CHUNKSIZE > TOTALDATACOUNT ? start + TOTALDATACOUNT % CHUNKSIZE : start + CHUNKSIZE;
                        let chunk: any = dataForInsert.slice(start, end)


                        Tag.insertMany(chunk, function (error: any, docs: any) {
                            if (error) {
                ...
                logger.info("after inserting the tags", new Date(), "validationData=>", validationData.length, "duplicateData=>", duplicateData.length);
    ...
    async createUniqueTags(tagsData: any) {

        try {
            let OFFSET = 0;
            let batchId_: any = '';
            let duplicateTagIds: any = [];
            let duplicateTagIdsErrorList: any = [];
            let duplicateSecureKey: any = [];
            let duplicateSecureKeyErrorList: any = [];
            let tagIdArray = [];
            let secureKeyArray = [];
            let updateRawDataArray: any = tagsData;
            batchId_ = tagsData?.[0]?.batchId;
            logger.debug("updateRawDataArray===>", updateRawDataArray.length)
            //     // check for duplicate tag ids
            let duplicateTagIdList: any = updateRawDataArray
                .map((e: any) => e['tagId'])
                .map((e: any, i: any, final: any) => final.indexOf(e) !== i && i)
                .filter((obj: any) => updateRawDataArray[obj])
                .map((e: any) => updateRawDataArray[e]);
            // DUPLICATE tags added into error list
            for (const iterator of duplicateTagIdList) {
                let duplicateDataMessage: any =
                {
                    tagId: iterator.tagId,
                    secureKey: iterator.secureKey,
                    message: 'Duplicate tag at tagId: ' + iterator.tagId,
                    errorType:'Validation'
                }

                duplicateTagIdsErrorList.push(duplicateDataMessage);

            }

            // removing duplicate tagIds from main array
            // updateRawDataArray = updateRawDataArray.filter((elem:any) => !duplicateTagIdList.find((element:any) => {
            // return element.tagId === elem.tagId})); 
            updateRawDataArray = updateRawDataArray
                .map((e: any) => e['tagId'])
                .map((e: any, i: any, final: any) => final.indexOf(e) === i && i)
                .filter((obj: any) => updateRawDataArray[obj])
                .map((e: any) => updateRawDataArray[e]);

            logger.debug("duplicateTagIdsErrorList after removing duplicate tags=>", duplicateTagIdsErrorList.length, updateRawDataArray.length)

            //     // check for duplicate secure keys
            let duplicateSecureKeyList: any = updateRawDataArray
                .map((e: any) => e['secureKey'])
                .map((e: any, i: any, final: any) => final.indexOf(e) !== i && i)
                .filter((obj: any) => updateRawDataArray[obj])
                .map((e: any) => updateRawDataArray[e]);

            //     // inserting duplicate secure keys
            for (const iterator of duplicateSecureKeyList) {
                let duplicateSecureKeyDataMessage: any =
                {
                    tagId: iterator.tagId,
                    secureKey: iterator.secureKey,
                    message: 'Duplicate secureKey at tagId: ' + iterator.tagId + ' and secureKey: ' + iterator.secureKey,
                    errorType:'Validation'
                }
                duplicateTagIdsErrorList.push(duplicateSecureKeyDataMessage);
            }

            // removing duplicate secure key from main array
            updateRawDataArray = updateRawDataArray
                .map((e: any) => e['secureKey'])
                .map((e: any, i: any, final: any) => final.indexOf(e) === i && i)
                .filter((obj: any) => updateRawDataArray[obj])
            ...
            logger.debug("duplicate Secure Key ErrorList after removing duplicate tags=>", duplicateTagIdsErrorList.length, updateRawDataArray.length)



            logger.debug("Unfilltered Array ==>", updateRawDataArray.length);
            OFFSET = 0;
            let addedData: any = [];
            for (let start = 0; start < updateRawDataArray.length; start++) {
                batchId_ = updateRawDataArray[start].batchId;
                if (CHUNKSIZEUPLOAD * OFFSET === start) {
                    let end = start + CHUNKSIZEUPLOAD > updateRawDataArray.length ? start + updateRawDataArray.length % CHUNKSIZEUPLOAD : start + CHUNKSIZEUPLOAD;
                    let chunk12 = updateRawDataArray.slice(start, end);
                    let chunk = chunk12.map((item: any) => item.tagId)
                    let duplicateTagIdArray = await Tag.find({ tagId: { $in: chunk } }).limit(chunk.length);

                    for (const iterator of duplicateTagIdArray) {
                        let TagIdold = await updateRawDataArray.find((o: any) => o.tagId === iterator.tagId);
                        let duplicateTagIdDataMessage: any =
                        {
                            tagId: iterator.tagId,
                            secureKey: TagIdold.secureKey,
                            message: 'Duplicate tag at tagId: ' + iterator.tagId,
                            errorType:'alreadyExist'
                        }
                        let lastKeyFound = duplicateTagIdsErrorList.find((o: any) => (o.tagId === iterator.tagId && o.secureKey == TagIdold.secureKey && o.errorType!=='alreadyExist'));
                        if (lastKeyFound === undefined) {
                            duplicateTagIdsErrorList.push(duplicateTagIdDataMessage);
                        }



                    }
                    logger.debug("duplicateTagIdsErrorList Count After Unique Tags Check ===> ", duplicateTagIdsErrorList.length);
                    // for secure key
                    let secureKeyChunk = chunk12.map((item: any) => item.secureKey)
                    let chunkKeys = secureKeyChunk.filter(function (el: any) { return el != null; });
                    let duplicateSecureKeyResponse = await Tag.find({ secureKey: { $in: chunkKeys } }).limit(chunkKeys.length);
                    for (const iterator of duplicateSecureKeyResponse) {
                        duplicateSecureKey.push(iterator);
                        let secureKeyTagId = await updateRawDataArray.find((o: any) => o.secureKey === iterator.secureKey);
                        let duplicateSecureKeyDataMessage: any =
                        {
                            tagId: secureKeyTagId.tagId,
                            secureKey: iterator.secureKey,
                            message: 'Duplicate secureKey at tagId: ' + secureKeyTagId.tagId + ' and secureKey: ' + iterator.secureKey,
                            errorType:'alreadyExist'
                        }
                        let lastKeyFound = await duplicateTagIdsErrorList.find((o: any) =>  (o.secureKey == iterator.secureKey && o.tagId===secureKeyTagId.tagId && (o.errorType!=='alreadyExist' || o.errorType!=='Validation')));
                        if (lastKeyFound === undefined) {
                            duplicateTagIdsErrorList.push(duplicateSecureKeyDataMessage);
                        }
                    }
            ...
            logger.debug("duplicateTagIdsErrorList After For Loop ===> ",duplicateTagIdsErrorList.length)

            updateRawDataArray = updateRawDataArray.filter((elem: any) => !duplicateTagIdsErrorList.find((element: any) => {
                return (element.tagId === elem.tagId)
            }));
           
            logger.debug("updateRawDataArray After Removing Duplicate Keys ===> ",duplicateTagIdsErrorList.length, updateRawDataArray.length)
        
            let secureKeyCount = 0;
            let standardKeyCount = 0;
            for (const item of updateRawDataArray) {
                ...
                if ((item.secureKey) != null && (item.secureKey) != '') {
            ...
            return Promise.resolve({
                errorMsgData: duplicateTagIdsErrorList,
                secureKeyCount: secureKeyCount,
                standardKeyCount: standardKeyCount,
                dataForInsert: updateRawDataArray,
                addedData: addedData,
                batchId: batchId_
            });
        }
        catch (error: any) {
            logger.error("Error in creating unique data for tags=>", error);
        }
    ...
    async digitize(req: Request) {
        ...
        try {
            ...
            logger.error("Error in creating degitization record====>", error.message)
            return Promise.reject(error.message)
        }
    }
    ...
    async commonDataProcessing(req: Request) {
        ...
        try {
            ...
            logger.debug("Req body data inside commonDataProcessing ProcessTypeName....>>>", processName?.data?.result?.processTypeName)
            ...
            if (processTypeName == TRACKNTRACE) {
                ...
                if ((readData.data.tagInfo).length > 0) {
                    ...
                    for (let i = 0; i < (readData.data.tagInfo).length; i++) {
                        logger.debug("Tag Info inside CommonDataProcessing=>", readData.data.tagInfo[i])
                        ...
                        logger.debug("Before calling tnt")
    ...
    async trackNTrace(req: Request) {
        try {
            ...
            logger.debug("trackNTrace============================>", req.body)
            let trackNTraceTag
            ...
            logger.debug("trackNTraceExist=>", trackNTraceExist)
            ...
            logger.debug("Fresh Tags==========================>")
            const epcisDocument = new EPCISDocument();

            let epicsObject: any = await this.createEpicsObject(req.body, false);
            let mergedBody = { ...body, ...epicsObject };
            logger.debug("Fresh Tags==========================> 2")

            let objectEvent = await this.objectEvent(mergedBody);
            logger.debug("Fresh Tags==========================> 3", operationTime)

            epcisDocument.setCreationDate(operationTime).addEvent(objectEvent);
            mergedBody.epcisDocument = epcisDocument;
            logger.debug("Fresh Tags==========================> 4")

            // body.action = FullCount
            logger.debug("TNT First Time Insert =>", mergedBody)
            trackNTraceTag = await TnTData.create(mergedBody)
            return Promise.resolve(trackNTraceTag)

        } catch (error: any) {
            logger.error("Error at adding TNT data in main TNT function", error)
            return Promise.reject(error.message)
        }
    }
    ...
    async uploadFiletoS3(req: Request) {
        ...
        try {
            ...
            logger.error('Error in Uploading Tags File:', err)
    ...
    public async exportData(req: Request, res: Response) {
        try {
            ...
            logger.debug("Succesfully generate the data for export tags", exportResponse.length);
    ...
    async exportDataLoadMore(req: Request, res: Response) {
        try {
            ...
            logger.debug("Where====>", whereObj, offset, ExportChunkValue)
            //do {
            responseResult = [];
    ...
    async exportTagData(req: Request, res: Response) {
        try {
            ...
            console.log("Response from exportTagDataReamMore=>", data.length)
    ...
    async exportTagDataReamMore(req: Request, res: Response) {
        try {
            ...
            for (const rows of responseResult) {
                ...
                let tempObject: any =
                {
                    'Manufacture name': rows.manufacturerName,
                    'Tag ID': rows.tagId,
                    'Batch Id': rows.batchId,
                    'Tag Type': rows.tagType,
                    'Tag Class': rows.tagClass,
                    'Status': rows.status,
                    'Created By': rows.userName,
                    'Created Date': createdAtTimeUpdated,
                }
                exportResponse.push(tempObject);
            }
    ...
    async writeInvalidTagsCSV(externalData: any, name: any, tenantId: any, tenantName: any) {
        try {
            ...
            logger.error("error in writing xlsx file uploading file in s3", error)
            return Promise.reject(error.message)
        }
    }
