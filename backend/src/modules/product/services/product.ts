...
export class ProductService extends BaseService {
    ...
    public async updateFileProducts(upcs: any, data: any, tenantId: any, t: any) {
        try{
         ...
         if (dataForUpdate.length) {
            ...
            logger.debug("BEfore Data Insertion Call=>",new Date())
            await this.generateUpdateSQL(dataForUpdate);
            logger.debug("After Insertion=>",new Date())
    ...
    async generateUpdateSQL(dataArray:any){
        try{
            let sql = "";
            let chunkSize = 200;
            let index=0;
            // if(dataArray?.lenght>5000){
            //     chunkSize = 1000;
            // } 
            //dataArray.map(async (item:any, index: number) => {
            for (const item of dataArray) {
                ...
                if(index%chunkSize == 0){
                    logger.debug("Hello in before execution",new Date(), index)    
                    await sequelize.query(sql);
                    //await this.executeQuery(sql);
                    sql = "";
                    logger.debug("Hello in after execution",new Date(), index, "Sql=>",sql)
                }
                index++;
            } 
                //});
            logger.debug("Before loop  385 =>", new Date(), "Sql=>", index)
            //await this.executeQuery(sql);
            await sequelize.query(sql);
            logger.debug("After loop 387=>",new Date())
