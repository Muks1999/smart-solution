...
import { PRODUCT_HEADER } from '../utils/constant';
    ...
    uploadSingleXLSX(req, res, async function (err) {
      try {
        ...
        if (req.file !== undefined) {
          ...
          if(headers[0]!==PRODUCT_HEADER.UPC 
            || headers[1]!==PRODUCT_HEADER.SKU
            || headers[2]!==PRODUCT_HEADER.NAME
            || headers[3]!==PRODUCT_HEADER.DESCRIPTION
            || headers[4]!==PRODUCT_HEADER.EXPERIENCE_ID
            || headers[5]!==PRODUCT_HEADER.EXPERIENCE_STUDIO_ID
            || headers[6]!==PRODUCT_HEADER.EXPERIENCE_TENANT_ID
            || headers[7]!==PRODUCT_HEADER.MANUFACTURER
            || headers[8]!==PRODUCT_HEADER.TYPE
            || headers[9]!==PRODUCT_HEADER.CATEGORIES
            || headers[10]!==PRODUCT_HEADER.SUB_CATEGORIES
            || headers[11]!==PRODUCT_HEADER.PRICE
            || headers[12]!==PRODUCT_HEADER.COLOR
            || headers[13]!==PRODUCT_HEADER.SIZE
            || headers[14]!==PRODUCT_HEADER.IMAGES
            || headers[15]!==PRODUCT_HEADER.IMAGE_URL
            || headers[16]!==PRODUCT_HEADER.OTHER_ATTRIBUTES
            )
            {
        ...
        logger.error("Error in product upload controller", error)
        respHndlr.sendError(res, error);
      }
    });
  }
